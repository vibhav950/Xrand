...
LRESULT CALLBACK KeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!(flags & 0x1) && !(flags & 0x20) &&
        !(flags & 0x80) && (key != prevKey || key != prevPrevKey))
    {
        nUserEventsAdded++;
...
BOOL RandFastPoll(void)
{
    ...
    {
        bufPtr->size = 16;
        if (pBCryptGenRandom(hBCryptProv, bufPtr->bytes, bufPtr->size, 0) == ERROR_SUCCESS)
        {
...
BOOL RandSlowPoll(void)
{
...

    {
        ...
        if (jent_entropy_init() == 0)
        {
            ...
            if (collector)
            {
                ssize_t ret = jent_read_entropy(collector, (char *)bufPtr->bytes, bufPtr->size);
                if (ret > 0)
                {
    ...
    if (hWinNativeApi == NULL)
    {
        if ((hWinNativeApi = GetModuleHandle("ntdll.dll")) != NULL)
        {
            if (!(pNtQuerySystemInformation =
                      (NTQUERYSYSTEMINFORMATION)GetProcAddress(hWinNativeApi, "NtQuerySystemInformation")))
            {
                FreeLibrary(hWinNativeApi);
                hWinNativeApi = NULL;
            }
    ...
    /* Read data from the CoreTemp shared memory interface */
    {
        ...
        {
            if ((pCoreTempData = (CORE_TEMP_SHARED_DATA *)
