...
/* Section 10.2.1.2 */
status_t ctr_drbg_update(CTR_DRBG_STATE *state, const uint8_t *provided_data,
                         size_t data_len) {
...
/* Section 10.2.1.5.1 */
status_t ctr_drbg_generate(CTR_DRBG_STATE *state, uint8_t *out, size_t out_len,
                           const uint8_t *additional_input,
                           size_t additional_input_len) {
  ...
  if (SUCCESS != ctr_drbg_update(state, additional_input, additional_input_len))
    return FAILURE;
  ...
  while (!feof(f)) {
    ...
    while (!feof(f)) {
      ...
      if (read_hex(f, &personalization_string,
        ...
        return 1;
      assert(SUCCESS == ctr_drbg_init(&state, entropy_input,
      ...
      if (read_hex(f, &additional_input,
        ...
        return 1;
      assert(SUCCESS == ctr_drbg_reseed(&state, entropy_input, additional_input,
      ...
      for (i = 0; i < 2; ++i) { // generate twice and overwrite the first output
        ...
        assert(SUCCESS ==
               ctr_drbg_generate(&state, generate_output, returned_bits_length,
