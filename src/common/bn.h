...
/*
Modified for Xrand by vibhav950 on GitHub
The original file can be found at https://github.com/kokke/tiny-bignum-c
*/
...
#define bignum_is_even(x) (((x)->array[0] & 1u) == 0u)
#define bignum_is_odd(x) (!bignum_is_even(x))
#define bignum_set_lsb(x) ((x)->array[0] |= 1u)
#define bignum_set_msb(x) ((x)->array[BN_ARRAY_SIZE - 1] |= (DTYPE)1 << (BIW - 1))
#define bignum_words(x) ((x) / WORD_SIZE + ((x) % WORD_SIZE ? 1 : 0))

int bignum_lsb(struct bn *x);
int bignum_msb(struct bn *x);
void bignum_sqr(struct bn *x); /* x = x^2 */
int bignum_cmp_int(struct bn *x, uint64_t n); /* Returns LARGER, EQUAL or SMALLER */
void bignum_add_int(struct bn *a, struct bn *x, int n); /* x = a + n */
void bignum_sub_int(struct bn *a, struct bn *x, int n); /* x = a - n */
