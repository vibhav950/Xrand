...
**Note:** If you wish to add a new feature, open an issue before starting work to get comments from the community. Someone may already be working on the same thing, or there might be good reasons to why the feature doesn't exist.
...
1. Every file should contain a file header comment (along with the license text) in the following format
    ...
    */
   ```
2. Write your code with security in mind - read [this guide](https://github.com/veorq/cryptocoding) to learn about some basic cryptography coding practices. Avoid trying to implement basic cryptographic features on your own, and use the functionality already available within the library, or from the [OpenSSL API](https://www.openssl.org/docs/index.html).
3. Keep the code portable and maintaiable by enforicing strict adherence to the C standard. On `gcc` or `clang`, you should compile with the `-Wpedantic` and `-Wall` options.
4. Write well-formatted code conforming to the [Linux kernel coding style](https://www.kernel.org/doc/html/v4.10/process/coding-style.html), and provide comments wherever necessary to make it easier for developers to review your code. To further improve code transparency and readability, you should provide a description for each function through comments
